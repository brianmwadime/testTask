...
type Numerable = number | string | BigNumber | BigNumberEthers
...
export const formatBN = (val: Numerable, precision: Numerable = 2, dividerDecimals = undefined) => {
  return toBN(val, dividerDecimals).decimalPlaces(Number(precision), BigNumber.ROUND_HALF_DOWN).toString();
};

export const formatBNWithCommas = (val: Numerable, precision: Numerable = 2, dividerDecimals = undefined, lessSign = false) => {
  ...
  if (lessSign && numVal < targetMinValue && Number(val) > 0) {
    return `<0.${arrayFromN(numPrecision - 1).map(() => '0').join('')}1`;
  }
